...
class ContactRequest(Base):
    ...
    __table_args__ = (
        ...
        UniqueConstraint("connection_id", "contact_id", "tg_message_id"),
        Index(
...
class DeliveryReceipt(Base):
    """Only identifiers and delivery outcomes; never message bodies."""
    ...
    state: Mapped[str] = mapped_column(Text, default="sending")
...
class ReplyJob(Base):
    """Transactional scheduling outbox containing template codes, never bodies."""
...
class PdfToken(Base):
    ...
    user_id: Mapped[int] = mapped_column(ForeignKey("access_users.user_id", ondelete="CASCADE"))
