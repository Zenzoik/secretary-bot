...
Реализовано ядро Этапа 1: webhook с дедупликацией, hard filter, gate
(расписание, исключения, один ответ на окно), классификатор со словарным
fallback, отложенная отправка с проверкой «владелец ответил сам», dry-run с
превью владельцу и утренний список.
