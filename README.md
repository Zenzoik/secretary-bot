...
коротким сообщением через Business Connection; Telegram сам показывает внизу
сообщения системную подпись подключённого бота. Дополнительная подпись в тексте
не добавляется. Денежные вопросы попадают в утренний список. Тела сообщений не
сохраняются нигде.
