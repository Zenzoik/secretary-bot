...
Зашифрованное 48-часовое хранение и суточное саммари:
[`docs/stage-4-summary.md`](docs/stage-4-summary.md).

Закрытый onboarding заказчиков и мастер-панель описаны в
[`docs/stage-1.6-onboarding.md`](docs/stage-1.6-onboarding.md).

Персональный автоответчик для Telegram Business. В нерабочее время он отвечает
коротким сообщением через Business Connection. В режиме `bot` в конце текста
добавляется `— 🤖 Секретар`: она видна во всех клиентах. Telegram Desktop также
показывает внизу сообщения системную подпись подключённого бота, а мобильный
клиент может её не показывать. При явно включённом суточном саммари тексты
сообщений хранятся в PostgreSQL только в виде AES-256-GCM ciphertext не более
48 часов; при выключенном флаге тексты не сохраняются.
...
dry-run с превью владельцу, суточное саммари по диалогам и управление из личного
чата с ботом.

Новое подключение всегда стартует в `dry_run = true`: контакту не уходит ничего,
пока владелец не переключит режим.

Управление владельца выполняется обычной Telegram-клавиатурой. `/start` открывает
панель; slash-команды сохранены только как запасной интерфейс.

| Кнопка | Действие |
|---|---|
| `📊 Статус` | Состояние, режим и временная пауза |
| `⏸ Пауза` | Выбор паузы на 1, 3, 8 или 24 часа |
| `🗓 Сьогодні` | Агрегаты действий за локальные сутки |
| `⛔ Вимкнути` / `▶️ Увімкнути` | Немедленно остановить или вернуть работу |
| `⚠️ Увімкнути live` | Запросить переход из dry-run с отдельным подтверждением |

Настройки доставки Этапа 1.7 хранятся на подключении: `sender_identity=bot|owner`,
границы задержки владельца (по умолчанию 10–60 секунд), случайная задержка бота
(по умолчанию 5–60 секунд) и `mark_read=false`. Чтение вызывается только после
успешной отправки и только при включённой настройке. Подробности и доказательства
проверки: [`docs/stage-1.7-delivery.md`](docs/stage-1.7-delivery.md).

Если Telegram временно недоступен при доставке отложенной задачи, worker возвращает
её в Redis и делает до трёх попыток с интервалом 5 секунд. Остановка контейнера также
возвращает уже захваченный пакет задач в очередь.

Клавиатура меняется согласно сохранённому состоянию сценария. Inline-кнопки в
dry-run и Manage Bot одноразовые: после действия они исчезают, а сообщение
получает постоянную отметку о результате. Подробности и журнал живой проверки:
[`docs/stage-1.5-control-plane.md`](docs/stage-1.5-control-plane.md).
...
Без `OPENAI_API_KEY` или `ANTHROPIC_API_KEY` классификатор работает на словаре
ключевых слов: бот не падает и не молчит, просто теряет точность. При `auto`
OpenAI имеет приоритет; модель задаётся через `OPENAI_MODEL` (по умолчанию
`gpt-5-mini`). Генерация суточного саммари без LLM
не выполняется, при этом ответы контактам продолжают работать.
Ключ OpenAI API создаётся отдельно на платформе OpenAI: подписка ChatGPT сама
по себе не выдаёт API-кредиты. Запросы выполняются через Responses API с
`store=false`.

Для включения временного хранения нужен отдельный 32-байтовый ключ в URL-safe
Base64:

```dotenv
MESSAGE_ENCRYPTION_KEY=replace-with-random-32-byte-key
```

Ключ не хранится в PostgreSQL. Его потеря делает сохранённые ciphertext
нерасшифровываемыми; его замена требует сначала выключить хранение в Mini App,
чтобы удалить старые записи.
...
Секреты и ключ шифрования хранятся только в `.env`, который исключён из Git.
Код не пишет открытые тексты сообщений в логи, Redis или файлы.
