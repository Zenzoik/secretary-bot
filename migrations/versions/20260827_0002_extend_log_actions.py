...
def upgrade() -> None:
    op.drop_constraint(CONSTRAINT, "message_log", type_="check")
    op.create_check_constraint(CONSTRAINT, "message_log", f"action IN ({ACTION_SQL_LIST})")


def downgrade() -> None:
    op.execute(f"DELETE FROM message_log WHERE action NOT IN ({PREVIOUS_ACTIONS})")
    op.drop_constraint(CONSTRAINT, "message_log", type_="check")
    op.create_check_constraint(CONSTRAINT, "message_log", f"action IN ({PREVIOUS_ACTIONS})")
