...
владельца. Telegram Desktop показывает метку подключённого бота, но живой тест
показал, что мобильный клиент может её не отображать. Поэтому режим v2 «от бота»
означает Business-сообщение с короткой подписью `— 🤖 Секретар` в конце текста;
Desktop дополнительно сохраняет системную метку. Это не отдельный участник диалога.
Обычный `sendMessage(chat_id=…)` от имени самого бота доступен только в отдельном
чате с контактом, который ранее сам запустил бота.
...
1. **Личность отправителя.** `sendMessage(business_connection_id=…)` создаёт
   ...
   этого диалога невозможна. Desktop показывает нативную метку под сообщением,
   тогда как в мобильном клиенте живой тест её не показал. Поэтому для
   `sender_identity=bot` к шаблону добавляется суффикс `— 🤖 Секретар`; Desktop
   при этом продолжает показывать и нативную метку.
