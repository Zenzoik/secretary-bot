...
- `can_read_messages` — отметка прочитанным (опционально, в MVP не используем)
...
### 2.5 Открытые вопросы, закрываются на Этапе 0

1. **Требуется ли Telegram Premium** на аккаунте владельца. Официальная документация Telegram на 27.08.2026 прямо указывает, что подключённые business-боты доступны без Premium. Это дополнительно проверяется тестовым аккаунтом, чтобы исключить различия клиента или раскатки функции.
2. **Приходят ли исходящие сообщения самого владельца** как `business_message`. От этого зависит надёжность защиты «владелец ответил сам» (§6.4).
...
**Зависимость:** механика работает, только если исходящие владельца приходят в `business_message`. Проверяется на Этапе 0. Если не приходят — деградируем до эвристики «владелец был онлайн», надёжность падает, это надо явно проговорить с владельцем.
...
| Telegram Premium | $0 по официальной документации; проверяется на Этапе 0 |
| **Итого** | **~$8–18** |
