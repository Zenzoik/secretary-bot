...
владельца, но клиент показывает метку подключённого бота. Заказчик подтвердил, что
этой нативной метки достаточно: дополнительная подпись в теле не добавляется.
Поэтому режим v2 «от бота» означает Business-сообщение с системной меткой, а не
отдельного участника диалога. Обычный `sendMessage(chat_id=…)` от имени самого бота
...
1. **Личность отправителя.** `sendMessage(business_connection_id=…)` создаёт
   ...
   этого диалога невозможна. Решение для `sender_identity=bot`: нативная метка
   подключённого бота под сообщением; тело содержит только шаблон ответа.
